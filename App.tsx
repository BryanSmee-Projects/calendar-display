...
function App() {
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden text-slate-900 bg-white">
      {/* Top Navigation Bar */}
      <header className="flex-none bg-white border-b border-gray-200 z-10 px-4 py-3 flex items-center justify-between shadow-sm">
        ...
        <div className="flex items-center gap-2 sm:gap-4 bg-gray-100 p-1 rounded-lg">
          <button onClick={() => handleNavigate('prev')} className="p-1.5 hover:bg-white rounded-md shadow-sm transition-all"><ChevronLeft className="w-4 h-4" /></button>
          <button onClick={() => handleNavigate('today')} className="px-3 py-1 text-sm font-semibold hover:bg-white rounded-md transition-all">{t.app.today}</button>
          <button onClick={() => handleNavigate('next')} className="p-1.5 hover:bg-white rounded-md shadow-sm transition-all"><ChevronRight className="w-4 h-4" /></button>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-4">
          ...
          <div className="flex bg-gray-100 p-1 rounded-lg">
            <button
              ...
              title={t.app.month}
              className={`p-1.5 rounded-md transition-all ${viewMode === 'month' ? 'bg-white shadow-sm text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
            >
          ...
          <div className="relative group">
            <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors text-gray-600 flex items-center gap-1">
              <Globe className="w-5 h-5" />
              <span className="text-xs font-bold uppercase">{language}</span>
            </button>
            <div className="absolute right-0 top-full mt-1 bg-white border border-gray-100 shadow-lg rounded-lg p-1 hidden group-hover:block w-24 before:absolute before:-top-2 before:left-0 before:w-full before:h-2 before:content-['']">
              <button onClick={() => changeLanguage('en')} className={`w-full text-left px-3 py-1.5 text-sm rounded-md ${language === 'en' ? 'bg-blue-50 text-blue-600' : 'hover:bg-gray-50'}`}>English</button>
              <button onClick={() => changeLanguage('fr')} className={`w-full text-left px-3 py-1.5 text-sm rounded-md ${language === 'fr' ? 'bg-blue-50 text-blue-600' : 'hover:bg-gray-50'}`}>Français</button>
            </div>
          </div>
      ...
      <main className="flex-1 flex overflow-hidden relative">
        ...
        {/* Sidebar */}
        <aside className="w-64 bg-gray-50 border-r border-gray-200 flex-col p-4 hidden lg:flex">
          <div className="mb-6">
            <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100">
              <span className="text-4xl font-bold text-gray-800 block mb-1">{format(currentDate, 'd', { locale })}</span>
              <span className="text-gray-500 uppercase tracking-wider text-sm font-medium">{format(currentDate, 'EEEE', { locale })}</span>
            </div>
          </div>
            ...
            <div className="space-y-2">
              {sources.map(source => (
                <div key={source.id} className="flex items-center gap-3 group cursor-default">
                  <div
                    className={`w-3 h-3 rounded-full transition-transform ${source.enabled ? 'scale-100' : 'scale-0 border-2 border-gray-300'}`}
                    style={{ backgroundColor: source.enabled ? source.color : 'transparent' }}
                  />
                  <span className={`text-sm font-medium truncate ${source.enabled ? 'text-gray-700' : 'text-gray-400'}`}>
                    {source.name}
                  </span>
                </div>
              ))}
              {sources.length === 0 && (
                <p className="text-sm text-gray-400 italic">{t.settings.noSources}</p>
              )}
            </div>
          </div>
        ...
        {/* View Container */}
        <div className="flex-1 p-4 md:p-6 overflow-hidden bg-white">
          {viewMode === 'month' ? (
            <MonthView
        ...
        >
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-xl font-bold text-gray-900 leading-snug">{selectedEvent.summary}</h3>
              <button onClick={() => setSelectedEvent(null)} className="p-1 hover:bg-gray-100 rounded-full">
                <Settings className="w-5 h-5 rotate-45 text-gray-400" />
              </button>
            </div>

            <div className="space-y-3 text-sm text-gray-600">
              <div className="flex gap-2">
                <CalendarIcon className="w-4 h-4 mt-0.5 text-gray-400" />
                <div>
              ...
              {selectedEvent.location && (
                <div className="flex gap-2">
                  <div className="w-4 h-4 mt-0.5 text-gray-400">📍</div>
                  <p>{selectedEvent.location}</p>
                </div>
              )}

              {selectedEvent.description && (
                <div className="mt-4 p-3 bg-gray-50 rounded-lg border border-gray-100 text-gray-700 whitespace-pre-wrap max-h-40 overflow-y-auto">
                  {selectedEvent.description}
                </div>
              )}
            </div>
